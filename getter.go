...
func (gg *GitGetter) FetchAPIDef(spec *TykSourceSpec) (*apidef.APIDefinition, error) {
	switch spec.Type {
	case TYPE_APIDEF:
		return gg.fetchAPIDefinitionDirect(spec)
	case TYPE_OAI:
		return gg.fetchAPIDefinitionFromOAI(spec)
	default:
...
func (gg *GitGetter) fetchAPIDefinitionDirect(spec *TykSourceSpec) (*apidef.APIDefinition, error) {
	if gg.r == nil {
		return nil, errors.New("No repository in memory, fetch repo first")
	}
...
func (gg *GitGetter) fetchAPIDefinitionFromOAI(spec *TykSourceSpec) (*apidef.APIDefinition, error) {

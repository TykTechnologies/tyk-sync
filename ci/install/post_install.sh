...
cleanup() {
    ...
    if [ "${use_systemctl}" = "True" ]; then
        rm -f /lib/systemd/system/tyk-sync.service
    else
        rm -f /etc/init.d/tyk-sync
    fi
}
...
setupOwnership() {
    ...
    [ "${change_ownership}" = "True" ] && chown -R tyk:tyk /opt/tyk-sync
    # Config file should never be world-readable
    chmod 660 /opt/tyk-sync/
}
...
cleanInstall() {
    ...
    if [ "${use_systemctl}" = "False" ]; then
        if command -V chkconfig >/dev/null 2>&1; then
            chkconfig --add tyk-sync
            chkconfig tyk-sync on
        fi
        if command -V update-rc.d >/dev/null 2>&1; then
            update-rc.d tyk-sync defaults
        fi

        service tyk-sync restart ||:
    else
        printf "\033[32m Reload the service unit from disk\033[0m\n"
        systemctl daemon-reload ||:
        printf "\033[32m Unmask the service\033[0m\n"
        systemctl unmask tyk-sync ||:
        printf "\033[32m Set the preset flag for the service unit\033[0m\n"
        systemctl preset tyk-sync ||:
        printf "\033[32m Set the enabled flag for the service unit\033[0m\n"
        systemctl enable tyk-sync ||:
        systemctl restart tyk-sync ||:
    fi
}
